...
const AdminPharmacies = () => {
    const { pharmacies, updatePharmacyStatus } = useOutletContext();
    ...
    return (
        <div className="animate-in fade-in duration-300">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 relative z-10">
                <h2 className="text-2xl font-bold text-gray-800 tracking-tight">Pharmacy Network</h2>
                    ...
                    <table className="w-full text-left border-collapse min-w-[900px]">
                        <thead>
                            <tr className="bg-gray-50/80 text-gray-500 text-xs uppercase tracking-wider border-b border-gray-100">
                                <th className="px-6 py-4 font-bold">Pharmacy Details</th>
                            ...
                            {filteredPharmacies.length === 0 ? (
                                <tr>
                                    <td colSpan="5" className="px-6 py-12 text-center text-gray-500">
                                        <p className="font-medium text-gray-600">No pharmacies found matching criteria</p>
                                    </td>
                                </tr>
                            ) : (
                                filteredPharmacies.map(pharm => (
                                    <tr key={pharm.id} className={`hover:bg-gray-50/50 transition-colors group ${pharm.status === 'Pending' ? 'bg-amber-50/30' : ''}`}>
                                        <td className="px-6 py-4">
                                            <div className="flex flex-col">
                                                <span className="font-bold text-gray-800 text-base">{pharm.name}</span>
                                                <span className="text-xs text-gray-500 font-mono mt-0.5">{pharm.id}</span>
                                            </div>
                                        </td>
                                        <td className="px-6 py-4">
                                            <span className="font-mono text-sm font-semibold tracking-wide text-gray-600 bg-gray-100 px-2 py-1 rounded-md">{pharm.license}</span>
                                        </td>
                                        <td className="px-6 py-4 text-sm text-gray-600 font-medium">
                                            {pharm.contact}
                                        </td>
                                        <td className="px-6 py-4">
                                            <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold ${pharm.status === 'Active' ? 'bg-green-100 text-green-700' :
                                                    pharm.status === 'Pending' ? 'bg-amber-100 text-amber-700 border border-amber-200 shadow-sm' :
                                                        'bg-red-100 text-red-700'
                                                }`}>
                                                {pharm.status === 'Pending' && <span className="relative flex h-2 w-2 mr-1"><span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-amber-400 opacity-75"></span><span className="relative inline-flex rounded-full h-2 w-2 bg-amber-500"></span></span>}
                                                {pharm.status}
                                            </span>
                                        </td>
                                        <td className="px-6 py-4 text-right space-x-2">
                                            ...
                                            {pharm.status === 'Pending' && (
                                                <button
                                                    onClick={() => updatePharmacyStatus(pharm.id, 'Active')}
                                                ...
                                                >
                                                    Approve Network Join
                                                </button>
                                            )}

                                            {pharm.status === 'Active' && (
                                                <button
                                                    onClick={() => updatePharmacyStatus(pharm.id, 'Suspended')}
